...
ASFightLayer* ASFightLayer::createAsFightLayer(int _MainIndex,int _BotIndex,bool _isCritical,int _damage,int _playerNumber,
                                               int _moveCount,int _shengming,int _botMoveCount,int _botShengMing){
    ASFightLayer* FightLayer = new ASFightLayer();
    if (FightLayer){
        if (_isCritical)
...
void ASFightLayer::initBasicFightLayer(int _MainIndex,int _BotIndex,bool _isCritical,int _damage,int _playerNumber,int _moveCount,int _shengming,int _botMoveCount,int _botShengMing){
...
void ASFightLayer::activeSkillNameAnimation(){
    ...
    activeSkillName->setPosition(ccp(size.width*3/4, winSize.height/2));
    ...
    CCCallFunc* passiveSkillNameEffect = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::passiveSkillNameAnimation));
    CCSequence* seq = CCSequence::create(scaleDown,effect,passiveSkillNameEffect,NULL);
    activeSkillName->runAction(seq);
}

void ASFightLayer::passiveSkillNameAnimation(){
    ...
    passiveSkillName->setPosition(ccp(size.width*3/4, winSize.height/2));
    ...
    if (MainIndex == 6 && !isCritical) {
        fightAnimation = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::LuffyInAttack));
        LuffyPreAttack();
        seq = CCSequence::create(scaleDown,effect,fightAnimation,NULL);
    }else if(MainIndex == 1 && !isCritical){
        fightAnimation = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::IronManInAttack));
        IronManPreAttack();
        seq = CCSequence::create(scaleDown,effect,fightAnimation,NULL);
    }else if(MainIndex == 2 && !isCritical){
        fightAnimation = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::MikuInAttack));
        MikuPreAttack();
        seq = CCSequence::create(scaleDown,effect,fightAnimation,NULL);
    }else if(MainIndex == 4 && !isCritical){
        fightAnimation = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::SheldonInAttack));
        SheldonPreAttack();
        seq = CCSequence::create(scaleDown,effect,fightAnimation,NULL);
    }else if(MainIndex == 3 && !isCritical){
        fightAnimation = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::LichKingInAttack));
        LichKingPreAttack();
        seq = CCSequence::create(scaleDown,effect,fightAnimation,NULL);
    }else if(MainIndex == 5 && !isCritical){
        fightAnimation = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::BladeMasterInAttack));
    ...
    if (MainIndex == 6 && isCritical) {
        ...
        fightAnimation = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::IronManInAttackCritical));
        IronManPreAttackCritical();
        seq = CCSequence::create(scaleDown,effect,fightAnimation,NULL);
    }else if(MainIndex == 2 && isCritical){
        CCDelayTime* delay = CCDelayTime::create(0.9);
        fightAnimation = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::MikuInAttackCritical));
        ...
        fightAnimation = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::SheldonInAttackCritical));
        SheldonPreAttackCritical();
        seq = CCSequence::create(scaleDown,effect,fightAnimation,NULL);
    }else if(MainIndex == 3 && isCritical){
        fightAnimation = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::LichKingInAttackCritical));
        LichKingPreAttackCritical();
        seq = CCSequence::create(scaleDown,effect,fightAnimation,NULL);
    }else if(MainIndex == 5 && isCritical){
        fightAnimation = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::BladeMasterInAttackCritical));
...
void ASFightLayer::showHeroInformation(){
    //1. 我方人物头像
    ...
    MainImage->setPosition(ccp(size.width/9, winSize.height/9));
    addChild(MainImage,100);
    
    //2. 我方人物姓名
    string mainStr = "LV." + int2string(player1->Level) + " " + MainUser->nickName;
    ...
    mainName->setPosition(ccp(size.height*35.5/100,winSize.height*14/88));
    addChild(mainName,100);
    
    //2. hp
    string HPStr = "生命力:" + int2string(shengming);
    MainHP = CCLabelTTF::create(HPStr.c_str(), "Arial Rounded MT Bold", 30,CCSizeMake(size.width/3, 0),kCCTextAlignmentLeft);
    MainHP->setPosition(ccp(size.width*38.5/100, winSize.height*10/88));
    addChild(MainHP,100);
    
    //3. sp
    string SPStr = "行动力:" + int2string(moveCount);
    MainSP = CCLabelTTF::create(SPStr.c_str(), "Arial Rounded MT Bold", 30,CCSizeMake(size.width/2, 0),kCCTextAlignmentLeft);
    MainSP->setPosition(ccp(size.width*46.75/100, winSize.height*6/88));
    addChild(MainSP,100);
    ...
    BotImage->setPosition(ccp(size.width*8/9, winSize.height*8/9));
    addChild(BotImage,100);
    
    //6. 敌方人物姓名
    string botStr = "LV." + int2string(bot2->Level) + " " + bot2->botNickName;
...
void ASFightLayer::removeThis(CCNode* sender){  removeChild(sender,true);    }