...
void ASFightLayer::BladeMasterPreAttack(){
    ...
    blade->setPosition(ccp(size.width*29/50,winSize.height*25/50));
    addChild(blade,3);
...
void ASFightLayer::BladeMasterInAttack(){
    ...
    CCCallFunc* hit = CCCallFuncN::create(this, callfuncN_selector(ASFightLayer::BladeMasterHitEnemy));
    CCSequence* hitseq = CCSequence::create(moveSlowly,hit,NULL);
    skillEffect_fly->runAction(hitseq);
}

void ASFightLayer::BladeMasterHitEnemy(){
    ...
    hitEffect->setScaleY(2.5);
    hitEffect->setScaleX(-2.5);
