...
void ASBotFightLayer::LuffyPreAttack(){
...
void ASBotFightLayer::LuffyInAttack(){
    
    removeChild(blade);

    
    CCActionInterval* moveOut = CCMoveBy::create(1, ccp(-size.width*3, 0));
    CCCallFunc* attack = CCCallFuncN::create(this, callfuncN_selector(ASBotFightLayer::LuffyAttackAnimation));
    CCSequence* seq = CCSequence::create(moveOut,attack,NULL);
    CameraLayer->runAction(seq);
}

void ASBotFightLayer::LuffyAttackAnimation(){
    ...
    CCCallFunc* hit = CCCallFuncN::create(this, callfuncN_selector(ASBotFightLayer::LuffyHitEnemy));
...
void ASBotFightLayer::LuffyHitEnemy(){
