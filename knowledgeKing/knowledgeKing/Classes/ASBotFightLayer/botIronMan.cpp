...
void ASBotFightLayer::IronManPreAttack(){
    ...
    blade->setPosition(ccp(size.width*31/50,winSize.height*9.8/20));
    addChild(blade,3);
...
void ASBotFightLayer::IronManInAttack(){
    ...
    skillEffect_fly->setPosition(ccp(size.width*2/3,winSize.height/2));
    addChild(skillEffect_fly,4);
    
    //2.粒子移动
    CCActionInterval* moveSlowly = CCMoveBy::create(1, ccp(-size.width/20,0));
    CCCallFunc* hit = CCCallFuncN::create(this, callfuncN_selector(ASBotFightLayer::IronManHitEnemy));
    CCSequence* hitseq = CCSequence::create(moveSlowly,hit,NULL);
    skillEffect_fly->runAction(hitseq);
}

void ASBotFightLayer::IronManHitEnemy(){
    ...
    hitEffect->setPosition(ccp(size.width*15/50,winSize.height/2));
