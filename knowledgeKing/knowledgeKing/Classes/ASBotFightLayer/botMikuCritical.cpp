...
void ASBotFightLayer::MikuPreAttackCritical(){
    ...
    hitEffect->setPosition(ccp(size.width*3/4,winSize.height/2+120));
...
void ASBotFightLayer::MikuInAttackCritical(){
    
    //1.镜头向右侧移动
    CCActionInterval* moveOut = CCMoveBy::create(0.5, ccp(size.width*13/4, 0));
    CCCallFunc* attack = CCCallFuncN::create(this, callfuncN_selector(ASBotFightLayer::MikuBigAttack));
    CCSequence* seq = CCSequence::create(moveOut,attack,NULL);
    CameraLayer->runAction(seq);
}

void ASBotFightLayer::MikuBigAttack(){
    CCPoint pos[6] = {
    ...
    if (mikuCriticalCounter < 6){
        ...
        CCCallFunc* hit = CCCallFuncN::create(this, callfuncN_selector(ASBotFightLayer::MikuHitEnemyCritical));
        ...
        CCCallFuncN* goOn = CCCallFuncN::create(this, callfuncN_selector(ASBotFightLayer::backToMainGame));
        runAction(CCSequence::create(delay,goOn,NULL));
    }
}

void ASBotFightLayer::MikuHitEnemyCritical(){
    //1.击中特效
    CCSprite* hitEffect = CCSprite::createWithSpriteFrameName("Sheldon_hit_0.png");
    hitEffect->setScale(5);
    hitEffect->setOpacity(200);
    hitEffect->setPosition(ccp(size.width*8/20,winSize.height/2));
