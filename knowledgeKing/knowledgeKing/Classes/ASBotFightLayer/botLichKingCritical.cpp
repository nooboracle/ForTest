...
void ASBotFightLayer::LichKingPreAttackCritical(){
    ...
    blade->setScaleX(-5);
    blade->setScaleY(5);
    blade->setPosition(ccp(size.width*4/7, winSize.height/2));
    addChild(blade,2);
...
void ASBotFightLayer::LichKingHitEnemyCritical(){
    
    //1.击中特效
    CCSprite* hitEffect = CCSprite::createWithSpriteFrameName("Sheldon_hit_0.png");
    hitEffect->setScaleX(-3);
    hitEffect->setScaleY(3);
    hitEffect->setOpacity(200);
    hitEffect->setPosition(ccp(size.width*3/7,winSize.height/2));
    ...
    CCCallFunc* cube = CCCallFuncN::create(this, callfuncN_selector(ASBotFightLayer::IceCubeFall));
...
void ASBotFightLayer::IceCubeHit(){
    ...
    //3.2 0.5秒后剑出现
    CCDelayTime* delay = CCDelayTime::create(0.5);
    CCCallFunc* sword = CCCallFuncN::create(this, callfuncN_selector(ASBotFightLayer::SwordFall));
...
void ASBotFightLayer::removeSwordAndCube(){
    removeChild(sword);
...
void ASBotFightLayer::LichKingMinusHP(){
...
void ASBotFightLayer::InitLK(){
    ...
    MainHero->setPosition(ccp(-size.width/2, winSize.height/2));
    ...
    BotHero->setPosition(ccp(-size.width*7/2, winSize.height/2));
