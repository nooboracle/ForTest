...
void ASBotFightLayer::SheldonPreAttack(){
    ...
    //2.小老鼠
    CCPoint mousePosition[3] = {ccp(size.width/2,winSize.height/2),ccp(size.width*11/20, winSize.height*9/20),ccp(size.width*11.5/20, winSize.height*11/20)};
    for (int i = 0 ; i < 3 ; i++) {
...
void ASBotFightLayer::SheldonInAttack(){
    ...
    for (int i = 0 ; i < 3 ; i++) {
        ...
        CCCallFunc* hit = CCCallFuncN::create(this, callfuncN_selector(ASBotFightLayer::MouseStopMoving));
        CCSequence* seq = CCSequence::create(effect1,remove,hit,NULL);
        mouse[i]->runAction(seq);
    }
}

void ASBotFightLayer::MouseStopMoving(CCNode* sender){
    
    removeChild(mouse[0]);
    removeChild(mouse[1]);
    removeChild(mouse[2]);
    
    CCPoint mousePosition[3] = {ccp(size.width*3/4,winSize.height/2),ccp(size.width*16/20, winSize.height*9/20),ccp(size.width*16.5/20, winSize.height*11/20)};
    for (int i = 0 ; i < 3 ; i++) {
        mouse[i] = CCSprite::createWithSpriteFrameName("Sheldon_0.png");
        mouse[i]->setScaleX(-2);
        mouse[i]->setScaleY(2);
        ...
        if (i == 0) {
            ...
            CCCallFunc* hit = CCCallFuncN::create(this, callfuncN_selector(ASBotFightLayer::Bazinga));
...
void ASBotFightLayer::Bazinga(){
    ...
    CCCallFunc* hit = CCCallFuncN::create(this, callfuncN_selector(ASBotFightLayer::SheldonHitEnemy));
...
void ASBotFightLayer::SheldonHitEnemy(){
...
void ASBotFightLayer::SilentAnimation(){
    
    CCSprite* hitEffect = CCSprite::createWithSpriteFrameName("Sheldon_S_0.png");
    hitEffect->setScaleX(-3.5);
    hitEffect->setScaleY(3.5);
    hitEffect->setOpacity(200);
