...
void RankScene::goToRankListList(){
    if (tableIndex != 2) {
        ...
        tableIndex = 2;
        CCCallFunc* stableRankList = CCCallFuncN::create(this, callfuncN_selector(RankScene::stableRankList));
        CCActionInterval* firstHalf = CCOrbitCamera::create(0.35, 0.5, 0, 0, 90, 0, 0);
        CCCallFunc* middle = CCCallFuncN::create(this, callfuncN_selector(RankScene::middleProcessToListList));
        CCActionInterval* secondHalf = CCOrbitCamera::create(0.35, 0.5, 0, 270, 90, 0, 0);
        CCCallFunc* resetContent = CCCallFuncN::create(this, callfuncN_selector(RankScene::resetToListList));
        CCSequence* seq = CCSequence::create(stableRankList,firstHalf,middle,secondHalf,resetContent,NULL);
        layerToRoll->runAction(seq);
    }
}

void RankScene::stableRankList(){
...
void RankScene::middleProcessToListList(){
    ...
    rankName->setString("选择排行榜");
    
    //2.改变按钮优先级
    CCMenu* menu = (CCMenu*)layerToRoll->getChildByTag(10);
...
void RankScene::resetToListList(){
    ...
    layerToRoll->addChild(pScrollView,2,999);
    ...
    //5.禁用排行榜列表按钮
    //((CCMenuItemSprite*)(bottomMenu->getChildByTag(14)))->setEnabled(false);
}

void RankScene::ListListContent(){
    
    for(int i = 0 ; i < 8 ; i++)
        ContainerLayer->removeChildByTag(20+i);
    ...
    for (int i = 0 ; i < 8 ; i++) {
        ...
        CCSprite* userImageItem = CCSprite::createWithSpriteFrameName(userImage[i].c_str());
        userImageItem->setScale(0.5);
        userImageItem->setPosition(ccp(size.width*200/1120, size.height*2230/1120 - size.height*i*63/660+winDif/130*alpha));
        ContainerLayer->addChild(userImageItem,4,160+i);
        
        //3.题包名称
        CCLabelBMFont* name = CCLabelBMFont::create(tibaoName[i].c_str(),"listname.fnt",size.width*0/3,kCCTextAlignmentLeft);
        name->setScale(0.417);
        name->setPosition(ccp(size.width*520/1120, size.height*2230/1120 - size.height*i*63/660+winDif/130*alpha));
        ContainerLayer->addChild(name,4,260+i);
        ...
        CCMenuItemSprite* chooseItem = CCMenuItemSprite::create(chooseButtonSprite,chooseButtonSprite_s,this,menu_selector(RankScene::tibaoRankIsSelected));
        chooseItem->setScale(0.8);
        chooseItem->setPosition(ccp(size.width*940/1120, size.height*2230/1120 - size.height*i*63/660+winDif/130*alpha));
        chooseItem->setTag(560+i);
        chooseMenu->addChild(chooseItem);
    }
    chooseMenu->setPosition(CCPointZero);
...
void RankScene::tibaoRankIsSelected(CCNode* sender){
    tibaoType = ((CCMenuItemSprite*)sender)->getTag() - 560;
    getRankListHTTP();
}
